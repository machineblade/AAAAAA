...
export const expoIn = t => {
  // The reference function subtracts 0.001 from the result
...
// Easing manager
export class Ease {
  constructor(action, easingFunction, rate=0) {
    this.action = action;
  ...
  update(dt) {
    if (this.finished || !this.initialized) return this.to || 0;

    this.time += dt;
    const t = Math.min(this.time / this.duration, 1);
    const easedValue = this.easingFunction(t, this.rate);
    const interpolatedValue = interpolate(this.from, this.to, easedValue);
